...
version: "1.2.0"
description: "Analiza patrones de gasto multi-cloud, identifica recursos subutilizados, oportunidades de capacidad reservada y proporciona recomendaciones de ahorro accionables para AWS, Azure y GCP"
author: "SMOUJBOT"
...
Proporciona análisis de costos inteligente y recomendaciones de optimización en infraestructura AWS, Azure y GCP. Identifica patrones específicos de desperdicio y entrega oportunidades de ahorro cuantificadas con comandos de implementación.

**Casos de uso real:**

1. **Detección semanal de anomalías de costo** - Escanear los últimos 7 días de datos de facturación, marcar servicios con picos de costo >20% vs periodo anterior, identificar causas raíz (p. ej., volúmenes EBS no adjuntos, balanceadores de carga inactivos, VMs sobreaprovisionadas)
2. **Planificación de capacidad reservada** - Analizar uso histórico de instancias EC2, bases de datos RDS y VMs de Azure para recomendar compras de reservas de 1 o 3 años con porcentajes de cobertura precisos
3. **Optimización de tamaño de recursos** - Evaluar utilización de CPU/memoria de instancias en ejecución, recomendar oportunidades de reducción de tamaño con intervalos de confianza (p. ej., "t3.large → t3.medium ahorraría $142/mes con 95% de retención de rendimiento")
4. **Optimización de capas de almacenamiento** - Identificar objetos S3/Blob/Cloud Storage con patrones de acceso adecuados para capas de acceso infrecuente o archivo, calcular ahorros de transición de clases de almacenamiento
5. **Limpieza de recursos huérfanos** - Detectar volúmenes no adjuntos, snapshots obsoletos, Elastic IPs no utilizadas, buckets S3 vacíos, instancias Cloud SQL inactivas con métricas de antigüedad e impacto en costo
6. **Aplicación de cumplimiento de etiquetas** - Escanear recursos que faltan etiquetas obligatorias de asignación de costos (Environment, Owner, Project), generar informes para equipos de gobernanza
7. **Pronóstico de presupuesto** - Proyectar gasto de 30 días basado en tasa actual, alertar cuando se predigan excesos con intervalos de confianza
